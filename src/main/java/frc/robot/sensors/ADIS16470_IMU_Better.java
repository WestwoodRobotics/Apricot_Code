...
public class ADIS16470_IMU_Better implements AutoCloseable, NTSendable {
  ...
  private static class AcquireTask implements Runnable {
    private ADIS16470_IMU_Better imu;

    public AcquireTask(ADIS16470_IMU_Better imu) {
      this.imu = imu;
    }
   ...
   */
  public ADIS16470_IMU_Better(SPI.Port port, CalibrationTime cal_time) {
  ...
  public void reset(IMUAxis axis) {
    synchronized (this) {
      if(axis == IMUAxis.kX) {
        m_integ_angle_x = 0.0;
      }
      else if(axis == IMUAxis.kY) {
        m_integ_angle_y = 0.0;
      }
      else if(axis == IMUAxis.kZ) {
        m_integ_angle_z = 0.0;
      }
    }
  }
  ...
  private void acquire() {
    ...
    final int dataset_len = 27; // 18 data points + timestamp
   ...
   */
  public synchronized double getXAngularRate() {
   ...
   */
  public synchronized double getYAngularRate() {
   ...
   */
  public synchronized double getZAngularRate() {
